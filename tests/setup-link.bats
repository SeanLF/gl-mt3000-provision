...
setup() {
    ...
    : > "$UCI_STORE"
    printf '%s\n' "sqm.eth0.enabled=0" "mtkhnat.global.enable=1" "wireless.mt798112.country=CA" "network.wan.mtu=1500" >> "$UCI_STORE"
...
@test "apply: above the CAKE ceiling turns SQM off" {
    ...
    [ "$(cat "$SETUP_LINK_STATE")" = "off" ]
}

@test "off: disables SQM, enables offload, MTU 1500, fast buffers" {
    run sh "$SL" off
    [ "$(uget sqm.eth0.enabled)" = 0 ]
    [ "$(uget mtkhnat.global.enable)" = 1 ]
    [ "$(uget network.wan.mtu)" = 1500 ]
...
@test "boot: replays the saved apply" {
...
@test "test: idle ping missing from one phase is taken from the other" {
    export FAKE_IDLE_BROKEN=download FAKE_LOADED="46.0/72.8"
    run sh "$SL" test
    [[ "$output" == *"idle 20.0 ms"* ]]
    [[ "$output" == *"(+26 ms)"* ]]
}

@test "test: idle ping missing from both phases falls back to fping" {
...
@test "arrive: fast bloated cable link is shaped with the remembered docsis type" {
    echo "apply 100 100 docsis" > "$SETUP_LINK_STATE"
    FAKE_LOADED="46.0/72.8"
    ...
    [ "$(cat "$SETUP_LINK_STATE")" = "apply 105.2 106.40 docsis" ]
}
